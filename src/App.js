...
// Componentsimport React, { useState, useEffect } from 'react';
import { Routes, Route, useNavigate, useLocation, Navigate } from 'react-router-dom';
import axios from 'axios';
import { ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
    ...
    <ThemeProvider>
      <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
        ...
        <a href="https://wa.me/+5511940705013" target="_blank" rel="noopener noreferrer"
          className="fixed bottom-24 right-6 bg-green-500 text-white p-4 rounded-full shadow-xl hover:bg-green-600 transition-all duration-200 z-50">
          <svg className="h-7 w-7" fill="currentColor" viewBox="0 0 32 32">
      ...
      <nav className="bg-white p-2 shadow-md fixed bottom-0 left-0 w-full z-50 border-t border-gray-200">
        <div className="container mx-auto flex justify-around">
          ...
          <button onClick={() => navigate(`/${currentTenantId}/order-summary`)} className="text-[#e63946] flex flex-col items-center text-xs relative">
            ...
            Carrinho
            {cartCount > 0 && (
              <span className="absolute top-0 right-1 bg-red-500 text-white rounded-full text-[10px] px-1">
                {cartCount}
              </span>
            )}
          </button>
