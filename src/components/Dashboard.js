    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {tenants.length === 0 ? (
        ...
        <ul className="space-y-4">
          {tenants.map((tenant) => (
            <li key={tenant.tenantId} className="p-4 bg-white shadow rounded">
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {tenant.phone || '-'} | {tenant.cep || '-'} | {tenant.street || '-'}, {tenant.number || '-'}
                    </p>
                  </div>
                  <button onClick={() => startEdit(tenant)} className="bg-[#e63946] text-white px-3 py-1 rounded text-sm hover:bg-red-600">Editar</button>
                </div>
              )}
            </li>
          ))}
