...
const fetchMyTenant = async () => { try { const token = localStorage.getItem('token'); const res = await api.get('/tenants/me', { headers: { Authorization: Bearer ${token} }, }); setTenant(res.data); setForm({ name: res.data.name || '', phone: res.data.phone || '', address: { cep: res.data.address?.cep || '', street: res.data.address?.street || '', number: res.data.address?.number || '' } }); } catch (err) { console.error('Erro ao carregar pizzaria:', err); toast.error('Erro ao carregar informações da pizzaria.'); } };

useEffect(() => { if (user?.isAdmin && !user?.isSuperAdmin) { fetchMyTenant(); } }, [user]);
...
const handleCancel = () => { setEditMode(false); setForm({ name: tenant.name || '', phone: tenant.phone || '', address: { cep: tenant.address?.cep || '', street: tenant.address?.street || '', number: tenant.address?.number || '' } }); };

return ( <div className="p-6 max-w-3xl mx-auto"> <h1 className="text-2xl font-bold mb-6" style={{ color: primaryColor }}> Painel do Administrador </h1>
...
{!tenant ? (
    ...
    <div className="bg-white shadow p-4 rounded space-y-4">
      {editMode ? (
        <>
          <input name="name" value={form.name} onChange={handleChange} className="w-full border p-2 rounded" placeholder="Nome da pizzaria" />
          <input name="phone" value={form.phone} onChange={handleChange} className="w-full border p-2 rounded" placeholder="Telefone" />
          <input name="cep" value={form.address.cep} onChange={handleChange} className="w-full border p-2 rounded" placeholder="CEP" />
          <input name="street" value={form.address.street} onChange={handleChange} className="w-full border p-2 rounded" placeholder="Rua" />
          <input name="number" value={form.address.number} onChange={handleChange} className="w-full border p-2 rounded" placeholder="Número" />
        ...
        <div className="flex justify-between items-start">
          ...
          <button onClick={() => setEditMode(true)} className="bg-[#e63946] text-white px-3 py-1 rounded hover:bg-red-600 text-sm">
            Editar
          </button>
        </div>
      )}
